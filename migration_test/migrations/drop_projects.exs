...
defmodule CaosTsdb.MigrationTest.DropProjectsTest do
  use CaosTsdb.MigrationTest.MigrationCase, target_migration: 20161217162513

  test "Migration on empty DB", _context do
    assert_raise Mariaex.Error, "(1146): Table 'caos_tsdb_migration_test.projects' doesn't exist", fn ->
      projects = from(p in "projects", select: p.id) |> Repo.all
    end
  end
  ...
  def before_migration(20161213154617) do
    ...
    s1 = Repo.insert! %SeriesOld{ttl: 1, project_id: "id1", metric_name: "metric1", period: 3600}
    s2 = Repo.insert! %SeriesOld{ttl: 2, project_id: "id1", metric_name: "metric2", period: 3600}
    s3 = Repo.insert! %SeriesOld{ttl: 3, project_id: "id2", metric_name: "metric1", period: 3600}
    s4 = Repo.insert! %SeriesOld{ttl: 4, project_id: "id2", metric_name: "metric2", period: 86400}
  ...
  test "Series are kept associated to tags", _context do
    tags = Tag
